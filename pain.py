...
#
    ...
    u=[1e3]*(7*(n-1)+4)
#
    res=differential_evolution(simu,args=(n,),callback=conv,bounds=list(zip(l,u)),workers=-1,seed=69,polish=True,maxiter=1000, init='sobol')
    print(res)
    x=res.x
    for i in range(n):
        if i < n-1:
